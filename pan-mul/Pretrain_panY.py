...
p=int(Y.shape[0]*0.995)
train_indices = np.random.choice(Y.shape[0], p, replace=False)
#train_indices = train_indices.astype(int)
#train_X = X[train_indices,:,:,:]
train_Y = Y[train_indices,:,:]
train_L = L[train_indices]
test_indices = np.array(list(set(range(Y.shape[0])) - set(train_indices)))
...
weights = {
    'wc1': tf.get_variable('W0', shape=(3,3,1,32), initializer=tf.contrib.layers.xavier_initializer()), #(3,3,4,32)
...
def conv_netx(y, weights, biases):  
...
pred,fc_feats = conv_netx(y, weights, biases)
...
with tf.Session() as sess:
    ...
    summary_writer = tf.summary.FileWriter('./srcY/Output', sess.graph)
    print('here 4')
    for i in range(training_iters):
        for batch in range(len(train_Y)//batch_size):
            ...
            opt = sess.run(optimizer, feed_dict={y: batch_y, l: batch_l, keep_prob : 0.5})
            loss, acc = sess.run([cost, accuracy], feed_dict={y: batch_y,
        ...
        test_acc,valid_loss = sess.run([accuracy,cost], feed_dict={y: test_Y,l : test_L, keep_prob : 1})
...
with tf.Session() as sess:
                ...
                for ind in range(0,Y.shape[0]/batch_size): #processing the datapoints batchwise
		    ...
		    batch_y = Y[ind_s:ind_e,:,:,:]
                    ...
                    cfeats = sess.run(fc_feats, feed_dict={y: batch_y, l: batch_l, keep_prob : 1})
                ...
                sio.savemat('pan_features.mat', {'features':fc_features}) #saving
